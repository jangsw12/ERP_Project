        ...
        public async Task<(List<InventoryDto> Inventories, int TotalCount)> SearchAsync(int? itemId, int? warehouseId, string tranType, string refType, DateTime? dateFrom, DateTime? dateTo, int pageNumber, int pageSize)
        {
            ...
            var inventories = await _dbExecutor.ExecuteReaderAsync("Inventory_Q",
                cmd =>
                {
                    ...
                    cmd.Parameters.Add("@p_tran_type", SqlDbType.NVarChar, 15).Value = string.IsNullOrWhiteSpace(tranType) ? DBNull.Value : tranType;
                    cmd.Parameters.Add("@p_ref_type", SqlDbType.NVarChar, 20).Value = string.IsNullOrWhiteSpace(refType) ? DBNull.Value : refType;
                ...
                reader =>
                {
                    ...
                    return new InventoryDto
                    {
                        ...
                        RefId = reader["RefId"] != DBNull.Value ? Convert.ToInt32(reader["RefId"]) : (int?)null,
